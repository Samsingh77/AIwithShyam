  ...
  return (
    <main className="relative font-sans selection:bg-brand-accent selection:text-brand-black overflow-x-hidden bg-[#0a0a0a] text-white min-h-screen">
      ...
      <AnimatePresence mode="wait">
        {view === 'landing' && (
          <motion.div key="landing" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            <Hero />
            <AISuite />
            <Pricing user={user} onAuthClick={() => setView('auth')} onPurchase={handlePurchase} />
            <PhotographyShowcase />
            <Footer />
          </motion.div>
        )}
        ...
        {view === 'auth' && (
          <motion.div key="auth" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="min-h-screen flex items-center justify-center px-6 pt-20">
        ...
        {view === 'dashboard' && user && (
          <motion.div key="dashboard" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            <MasterDashboard user={user} onBackToLanding={() => setView('landing')} />
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <style>{`
        ...
        @media (min-width: 768px) { .border-text { -webkit-text-stroke: 2px rgba(255, 255, 255, 0.3); } }
      `}</style>
    </main>
  );
}
